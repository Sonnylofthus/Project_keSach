...
public class BookShelfResource implements IBookShelf {
    ...
    @Override
    public List<ReadBook> getBookshelfsBook(int BookShelfId) {
        ...
        try {
            ResultSet rs=connectSql.query("SELECT readbook_bookshelf.book_id,readbook_bookshelf.bookshelf_id,readbook.book_id,book.book_name,\n" +
                    ...
                    "join book on readbook.book_id=book.book_id and readbook_bookshelf.bookshelf_id="+BookShelfId);
            while(rs.next()){
    ...
    @Override
    public int addBookToBookShelf(int bookSelfId, long bId) {
        int count = 0;
        try {
    ...
    @Override
    public int removeBookFromBookShelf(int bookSelfId, long bId) {
        int count = 0;
        try {
